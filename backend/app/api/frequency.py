...
def dashboard(_: dict = Depends(require_admin)) -> dict:
    try:
        ...
        logger.exception("Falha ao ler o cache de vencimentos")
        raise HTTPException(status_code=503, detail="Não foi possível consultar os vencimentos dos planos.") from exc
...
def refresh(user: dict = Depends(require_csrf)) -> dict:
    ...
    try:
        ...
        logger.exception("Atualização manual de vencimentos falhou")
        raise HTTPException(
            status_code=503,
